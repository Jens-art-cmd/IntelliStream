...
export default async function AdminStatusPage() {
  ...
  // Pro Branche: letzter Scout-Lauf + Artikel heute/Rückstau
  ...
  const statusIcon = (hoursAgo: number | null) => {
    if (hoursAgo === null) return <AlertTriangle size={14} color="#C0392B" />;
    if (hoursAgo <= 2)     return <CheckCircle   size={14} color="#2D7553" />;
    if (hoursAgo <= 6)     return <Clock         size={14} color="#E08900" />;
    return                        <AlertTriangle  size={14} color="#C0392B" />;
  };
    ...
    <div className="max-w-5xl mx-auto px-6 py-7">
      ...
      <div className="rounded-xl overflow-hidden mb-6" style={{ background: "#FFFFFF", border: "1px solid #E2DDD2" }}>
        <div className="px-5 py-4" style={{ borderBottom: "1px solid #EBE7DD", background: "#FAF8F4" }}>
          <h2 className="text-sm font-semibold" style={{ color: "#1A1813" }}>Scout-Status je Branche</h2>
          <p className="text-xs mt-0.5" style={{ color: "#8C887E" }}>Zuletzt eingesammelter Artikel pro Branche</p>
        </div>

        {/* Kopfzeile */}
        <div
          className="grid px-5 py-2 text-2xs font-bold uppercase"
          style={{
            gridTemplateColumns: "1fr 120px 80px 80px 90px",
        ...
        <div className="divide-y" style={{ borderColor: "#F1EDE4" }}>
          {industryStats.map(ind => (
            <div
              ...
              style={{ gridTemplateColumns: "1fr 120px 80px 80px 90px" }}
            >
              ...
              <span style={{ color: ind.hoursAgo === null ? "#C0392B" : ind.hoursAgo <= 2 ? "#2D7553" : "#E08900" }}>
                {ind.hoursAgo === null
                  ? "Nie"
                  : ind.hoursAgo === 0
                    ? "Gerade eben"
              ...
              <span style={{ color: ind.backlog > 50 ? "#C0392B" : ind.backlog > 0 ? "#E08900" : "#2D7553" }}>
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: "#FEF0EE", border: "1px solid #F5C6C1" }}>
          ...
          <div className="divide-y" style={{ borderColor: "#F5C6C1" }}>
            {(sources ?? [])
              ...
              .map(s => (
                <div key={s.id} className="px-5 py-3 flex items-center justify-between text-xs">
